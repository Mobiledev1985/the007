...
export default class WebService {
    ...
    static callServicePost = (URL, apiBody, callBackFunction) => {
        ...
        .then(async responseData => {
            ...
            try {
                let tmpJson = await JSON.parse(responseData);
                console.log('responseData>>>>>>>>>>>>>', URL, "responseData success-----", tmpJson);
                // console.log('responseData>>', responseData);
        ...
        .catch(error => {
            ...
            if (error.message?.includes('JSON')) {
                // Do nothing
            }else if (error.message == Constants.ERROR_COMETCHAT_LOGOUT) {
                callBackFunction(Constants.ERROR_MESSAGE_COMETCHAT_LOGOUT, true);
            } else if (error.message == Constants.AUTHENTICATE_ERROR) {
                //callBackFunction(Constants.NO_INTERNET, true);
                callBackFunction(Constants.ERROR_401, true);
                AsyncStorage.setItem(Constants.KEY_USER_ID, "")
                .then(() => AsyncStorage.setItem(Constants.KEY_USER_TOKEN, ""))
