...
import { BASE_URL, BASE_URL_DEV } from '../consts/Global';
...
class FirebaseChatService {
    ...
    getRecentChat = (userId, callback, startAfter = undefined) => {
        ...
        if (startAfter) {
            firestore_unsub = firestore_unsub.startAfter(startAfter)
        }
            
        firestore_unsub = firestore_unsub.onSnapshot(async (querySnapshot) => {
                console.log('getRecentChat   ------------ got result  ',querySnapshot.size)
                if (querySnapshot ) {
                    chatDataList = [];
                    ...
                    let uri = Memory().env == "LIVE" ? BASE_URL + 'header'  : BASE_URL_DEV + 'header';

                    let params = {}
                    
                    params["token"] = await AsyncStorage.getItem(Constants.KEY_USER_TOKEN)
                    params["user_id"] = await AsyncStorage.getItem(Constants.KEY_USER_ID)
                    params["format"] = "json";
                    console.log("header params:", params)
                    WebService.callServicePost(uri, params, (response, isError) => {
                        console.log("header:", response, "isError:",isError)
                        let unreadMsgCountPerChannel = response.data.unreadMsgCountPerChannel
                        querySnapshot.docChanges().forEach(async function (chatData) {
                            ...
                            if (chatData.type === "added") {
                                ...
                                try{
                                    ...
                                    {
                                        data.newMsgCnt = unreadMsgCountPerChannel[chatData.doc.id] ?? 0
...
//                                 }
    ...
    updateMsgToRead = (msg, selectedData) => {
        ...
        callChatRead(selectedData.grpId)
    ...
    getChatRoom = (userId, memberUserId, callback) => {
        let chatDataList = [];
        let count = 0;
                ...
                if (querySnapshot ) {
                    ...
                    querySnapshot.docChanges().forEach(async function (chatData) {
                        ...
                        if (chatData.type === "added") {
                            ...
                            var data = chatData.doc.data();
                            // console.log("data.members_user_id", data.members_user_id)
                            const isCorrectChat = data.members_user_id.length === 2 && data.members_user_id.includes(memberUserId) && data.members_user_id.includes(userId)
                            ...
                            try{
                                if(chatData.doc && chatData.doc.id && isCorrectChat)
                                {
                                    let newMsgs = await firestore().collection(dbName2)
...
const callChatRead = async (grpId) => {
    console.log('---------------------------')
    try {
        let uri = Memory().env == "LIVE" ? BASE_URL + 'update-old-chat-read' : BASE_URL_DEV +  'update-old-chat-read';
        ...
        params["format"] = "json";
        WebService.callServicePost(uri, params, (response, isError) => {
            console.log("callChatRead:", response, "isError:",isError)
