...
export default class AddCoin extends React.Component {
    ...
    UNSAFE_componentWillMount() {
        ...
        layoutChangeDetector = Dimensions.addEventListener("change", () => {
            if (Dimensions.get("window").width < Dimensions.get("window").height) {
                this.setState({
                    is_portrait: true,
                })
    ...
    purchaseGoldCoinPaypal(pay_type) {
        try {
            ...
            params["token"] = this.state.userToken;
            ...
            params["save_data"] = JSON.stringify(save_data);
            params["send_data"] = JSON.stringify(send_data);
                                ...
                                <View style={{ width: '100%', zIndex: 100 }}>
                                    ...
                                    <View style={{ width: '100%', height: 30, marginTop: 5, borderWidth: 0.5, borderColor: Colors.black, borderRadius: 5, flexDirection: 'row' }}>
                                        ...
                                        <View style={{ flex: 1, height: '100%' }}>
                                            ...
                                            /> */}
                                            <TextInput style={[{ width: '100%', paddingLeft: 5, paddingTop: 5, height: '100%' }, stylesGlobal.font,]} autoComplete={'cc-number'} keyboardType = {Platform.OS === 'ios' ? 'ascii-capable' : 'visible-password'}  textContentType={'creditCardNumber'} placeholder='1234 5678 9012 3456' onChangeText={(text) => this.onChangeCardNumberText(text)}>{this.state.card_number}</TextInput>
                                        </View>
                                    </View>
                                </View>
