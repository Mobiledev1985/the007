...
export default function App() {
    ...
    const TAG = "APP ROUTE";
     ...
     async function setNotificationBadge(data) {
        ...
        //killed app notification tap handling as per new notifee documentation
        useEffect(() => {
            return notifee.onForegroundEvent(({ type, detail }) => {
                if(Platform.OS === "ios"){
                    if(type === EventType.PRESS && detail.notification){
                        if(detail.notification?.data?.toPage && detail.notification?.data?.eventId && Number(detail.notification?.data?.toPage) == 1){
                            RootNavigation.navigate("EventDetail", {
                                eventId: detail.notification?.data?.eventId,
                                refreshEventData: () => {},
                                EventCategory: detail.notification?.data?.eventCatId == "10" ? "travel" : "party"
                            })
                        } else if(detail.notification?.data?.toPage && Number(detail.notification?.data?.toPage) == 6){
                            // RootNavigation.navigate('Dashboard', {logoclick: true, selected_screen: "chat"});
                            setTimeout(() => {
                                // RootNavigation.navigate('Dashboard', {logoclick: false, selected_screen: "chat"});
                                EventRegister.emit("goto_tab_recentchat", '');
                            }, 1000);
                        }
                    }
                }
    ...
    useEffect(() => {
        ...
        //notification handling in foreground state (Android + iOS)
        function onNotification(notify, data) {
            ...
            if(data.toPage == "6" && data.chatGrpId == activeChatGrpId){
                console.log("Notification for active chat screen not need to display-------");
            } else {
        ...
        function onOpenNotification(notify) {
            ...
            if(notify) {
                const routeName = RootNavigation.getCurrentRoute();
                    //killed app notification tap handling for android
                    if(notify.data?.toPage && Number(notify.data?.toPage) == 6){
                        setTimeout(() => {
                            // RootNavigation.navigate('Dashboard', {logoclick: false, selected_screen: "chat"});
                            EventRegister.emit("goto_tab_recentchat", '');
                        }, 2000);
                    } else if(notify.data?.toPage && notify.data?.eventId && Number(notify.data?.toPage) == 1){
                        setTimeout(() => {
                            RootNavigation.navigate("EventDetail", {
                                eventId: notify.data?.eventId,
                                refreshEventData: () => {},
                                EventCategory: notify.data?.eventCatId == "10" ? "travel" : "party"
                            })
                        }, 2000);
                    }
            }
        ...
        const appStateDetector = AppState.addEventListener('change', async(state) => this.handleAppStateChange(state));
        this.listener = EventRegister.addEventListener("active_chat_grpid_hide_notification", (data) => {
            activeChatGrpId = data;
        })
...
if (!String.prototype.replaceAll) {
    String.prototype.replaceAll = function (search, replacement) {
      if (typeof search === 'string') {
        ...
        // If search is a regular expression, convert it to a string
        search = search.source;
      } else {
        throw new TypeError('First argument must be a string or regular expression');
      }
  
      // Create a new regular expression with the global flag
      var regex = new RegExp(search, 'g');
  
      // Use String.replace method with the new regex
